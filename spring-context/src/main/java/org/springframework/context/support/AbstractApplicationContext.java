...
public abstract class AbstractApplicationContext
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				 ...
				 *
				 *
				 *   registerBeanPostProcessors方法主要是将BeanDefinition对应的BeanPostProcessor实例化并通过
				 *   beanFactory.addBeanPostProcessor()方法注册进来。前面分析过AnnotationConfigUtils.registerAnnotationConfigProcessors
				 *   会向容器注册几个Spring内置的BeanPostProcessor，这步主要是将应用中引入的BeanPostProcessor注册进来。
				 *
				 * 上步invokeBeanFactoryPostProcessors执行完成后，Spring会将所有的Bean解析成BeanDefinition注册到容器中，
				 * 其中就可能包含BeanPostProcessor的BeanDefinition信息，这个方法就是把这些BeanPostProcessor对应的BeanDefinition通过getBean方式实例化，
				 * 并通过addBeanPostProcessor()注册进来，这样这些BeanPostProcessor才能起作用。
				 *
				 * 和上步invokeBeanFactoryPostProcessors不同的是，这里只是把所有的BeanPostProcessor注册进来，并没有去执行，
				 * 因为这也很好理解：BeanPostProcessor是围绕在Bean实例化周围的扩展点，这里服务Bean存储在容器中基本都还是BeanDefinition，还没有进行实例化。
				 */
				 ...
				 *
				 *
				 * onRefresh默认是空实现，模板模式设计主要用于子类扩展。
				 * 可以参照SpringBoot中ServletWebServerApplicationContext这个类，重写了onRefresh()方法，
				 * 在这个方法中完成内嵌Servlet容器的创建：Tomcat、Jetty、Undertow，将程序内嵌一个Servlet容器后，就可以独立运行。
				 */
				onRefresh();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		*/
		...
		// 增加一个bean的后置处理器，ApplicationListenerDetector 将类型是ApplicationListener的bean添加到事件广播器，以便触发事件时被调用
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		// 执行Bean工厂的所有后置处理器
		//getBeanFactoryPostProcessors()获取到ApplicationContext.beanFactoryPostProcessors集合中存储的BeanFactoryPostProcessor，
